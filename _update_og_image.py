...
Add: og:image:secure_url, og:image:type, og:image:width, og:image:height
"""
import os
import re
import json
...
for filename in html_files:
    ...
    if not match:
        # No og:image tag found, skip
        continue
    
    og_image_tag = match.group(1)
    og_image_url = match.group(2)
    
    # 5. Extract filename from URL
    og_image_filename = os.path.basename(og_image_url)
    
    # 6. Determine JPG and WebP filenames
    ...
    # 8. Build base URL (without filename)
...
<meta property="og:image:height" content="{height}">'''
    ...
    # 11. Check if already has og:image:width (avoid duplicate)
    if 'og:image:width' in new_content and og_image_tag not in new_content:
