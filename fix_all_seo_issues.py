...
print("=== 修复 P2 #8: 添加 article:modified_time ===\n")
...
for filename in article_pages:
    ...
    def add_modified_time(match):
        return f'{match.group(1)}\n<meta property="article:modified_time" content="{current_date}">'
    
    new_content = re.sub(pattern, add_modified_time, content)
...
print(f"\n总计添加: {fixed_stats['article_modified_time']} 个页面\n")

# ========== 修复 2: 修复 sitemap.xml changefreq ==========
print("=== 修复 P2 #7: 调整 sitemap.xml changefreq ===\n")
...
for page in static_pages:
    # 将 weekly 改为 monthly
    pattern = rf'(<loc>https://golightly\.fun/{page}</loc><lastmod>[\d-]+</lastmod><changefreq>)weekly(</changefreq>)'
    ...
    if new_sitemap != sitemap:
        ...
        print(f'✅ {page}: weekly -> monthly')

with open(sitemap_path, 'w', encoding='utf-8') as f:
    f.write(sitemap)

print(f"\n总计修改: {fixed_stats['sitemap_changefreq']} 个 URL\n")

# ========== 修复 3: 添加 BreadcrumbList JSON-LD ==========
print("=== 修复 P1 #6: 添加 BreadcrumbList 结构化数据 ===\n")
...
for filename, items in breadcrumb_data.items():
    ...
    if 'BreadcrumbList' in content:
        print(f'⚠️  {filename}: 已有 BreadcrumbList')
        continue
    ...
    # 插入到第一个 script type="application/ld+json" 前
    json_str = f'<script type="application/ld+json">\n{json.dumps(breadcrumb_json, ensure_ascii=False, indent=2)}\n</script>'
...
print(f"\n总计添加: {fixed_stats['breadcrumb_schema']} 个页面\n")

# ========== 修复 4: 外部联盟链接添加 nofollow sponsored ==========
print("=== 修复 P2 #10: 外部联盟链接添加 rel=\"nofollow sponsored\" ===\n")
...
print(f"\n总计修改: {fixed_stats['nofollow_sponsored']} 个联盟链接\n")
...
print("\n✅ 所有 P1 + P2 问题修复完成!")
print("⚠️  部分问题需要手动检查:")
print("  1. P1 #4: 内链结构 - 需要手动添加上下文相关内链")
print("  2. P1 #5: 关键词密度 - 需要手动优化内容")
print("  3. P0 #3: 图片 Alt 属性 - 需要进一步检查")
