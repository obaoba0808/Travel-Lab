...
# Correct positions from clean 0d2c549 files
INSERT_POSITIONS = {
    'esim-comparison.html': 17249,    # after </div> before <!-- FOOTER -->
    'live-japan-budget.html': 407266,  # after </div><!-- /article-container -->
}
...
for fname, insert_pos in INSERT_POSITIONS.items():
    path = os.path.join(base, fname)
    # Start fresh from clean version
    ...
    ctx = orig[insert_pos-5:insert_pos+80]
    print(f'\n=== {fname} ===')
    ...
    # Verify in MODIFIED file
    form_idx = c.find('class="lead-inline"')
    foot_idx = c.find('<footer')
    print(f'Form@{form_idx}, Footer@{foot_idx}')
    print(f'Before footer? {form_idx < foot_idx}')
    
    # Check what's around the form
    print(f'Around form: {repr(c[form_idx-50:form_idx+50])}')
