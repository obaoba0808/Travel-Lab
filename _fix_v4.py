...
for fname, pdf_key in PAGE_PDF_MAP.items():
    ...
    # Step 1: Get TRULY clean version - reset to HEAD~2 via git checkout
    import subprocess
    ...
    # Step 2: Find LAST </main> occurrence (the one that closes the main content area)
    last_main_end = c.rfind('</main>')
    
    if last_main_end == -1:
        # Try other structural markers
        markers = ['</article>', '<footer', '<div class="footer"', '</body>']
        inserted = False
        for m in markers:
            idx = c.rfind(m)
            if idx > len(c) // 2:  # must be in the second half of the file
                c = c[:idx] + '\n' + FORM_HTML + '\n' + c[idx:]
                inserted = True
                break
        if not inserted:
            print(f"SKIP (no structure): {fname}")
            continue
    else:
        # Insert form right before </main>
        c = c[:last_main_end] + '\n' + FORM_HTML + '\n' + c[last_main_end:]
    
    # Step 3: Insert CSS in <head> (before </head>)
    head_end = c.find('</head>')
    if head_end != -1:
        c = c[:head_end] + CSS_BLOCK + '\n' + c[head_end:]
    
    # Step 4: Insert JS before </body>
    ...
    # Verify: find the HTML form (not CSS)
    html_form_idx = c.find('class="lead-inline"')
    main_idx = c.rfind('</main>')
    head_idx = c.find('</head>')
    
    status = "OK"
    if html_form_idx < head_idx:
        status = "WARNING: in HEAD!"
    elif html_form_idx > main_idx:
        status = "WARNING: after main!"
    
    modified.append(fname)
    print(f"{status}: {fname} (form@{html_form_idx}, </head>@{head_idx}, </main>@{main_idx})")
