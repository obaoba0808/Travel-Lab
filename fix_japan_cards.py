#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""在 japan-travel.html 的 card-grid 里添加缺失的 3 篇文章卡片（精确处理 CRLF）"""
...
print("检查状态：")
print(f"  - 大阪美食攻略卡片: {'✅ 已存在' if has_osaka_food else '❌ 缺失'}")
print(f"  - 大阪USJ环球影城卡片: {'✅ 已存在' if has_osaka_usj else '❌ 缺失'}")
print(f"  - 日本预算指南卡片: {'✅ 已存在' if has_japan_budget else '❌ 缺失'}")

if has_osaka_food and has_osaka_usj and has_japan_budget:
    print("\n[SKIP] 3 篇文章卡片已存在")
else:
    # 找到 card-grid 里最后一个 </article> 的位置
    # 先找到 <div class="card-grid"> 的位置
    card_grid_start = content.find('<div class="card-grid">')
    
    if card_grid_start == -1:
        print("[ERROR] 找不到 card-grid")
    else:
        # 从 card_grid_start 开始，找到最后一个 </article>
        search_start = card_grid_start
        last_article_end = -1
        while True:
            pos = content.find('</article>', search_start)
            if pos == -1:
                break
            last_article_end = pos
            search_start = pos + len('</article>')
        
        if last_article_end == -1:
            print("[ERROR] 找不到 card-grid 里的 </article>")
        else:
            # 插入位置：last_article_end + len('</article>')
