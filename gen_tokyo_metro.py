...
# Cover
story += make_cover('東京地鐵路線圖', '2026 最新版')

# Section 1
story.append(Paragraph('東京地鐵完整指南', h1))
story.append(Paragraph('東京擁有13條地鐵路線、285個車站，是全世界最複雜的城市軌道交通系統之一。本指南提供完整路線圖、轉乘車站、票價資訊與旅遊建議，幫助您在東京暢行無阻。', body))
...
story.append(make_table(['路線名稱', '代表色', '車站數', '營運長度(km)'], data))
story.append(Spacer(1, 10))

# Section 2
story.append(Paragraph('票價與優惠票券', h2))
story.append(Paragraph('東京地鐵票價依距離計算，成人單程票價格介於170-320日圓之間。建議購買「東京地鐵24小時券」（800日圓）或「都營地鐵+東京地鐵共通券」（1,590日圓/24小時），可無限次搭乘。', body))
story.append(Spacer(1, 10))
