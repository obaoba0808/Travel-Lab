...
# Pages that need form position fix (have </main>, form is inside main)
# We need to move form to before <footer> instead
TARGET_PAGES = [
...
for fname in TARGET_PAGES:
    ...
    # Reset to truly clean version (before ANY form was added)
    ...
    # Insert form BEFORE <footer> (not before </main>)
    # This ensures form is the last element before footer, always correct
    ...
    # Insert form right before <footer>
    c = c[:footer_idx] + '\n' + FORM_HTML + '\n' + c[footer_idx:]
    ...
    foot = c.find('<footer')
    print(f"OK: {fname} (form@{fi}, footer@{foot}, before_footer={fi<foot})")
