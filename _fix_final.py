...
for fname, pdf_key in PAGE_PDF_MAP.items():
    ...
    # CRITICAL: Get the CLEAN version from git (before ANY of our form changes)
    # HEAD~2 = a749c13 (CORS fix) - before the 11-page form addition
    ...
    if result.returncode != 0:
        print(f"SKIP (git show failed): {fname}")
        continue
    ...
    ctx_before = c[max(0,idx-60):idx]
    has_main_before = '</main>' in c[idx+200:]  # </main> should be AFTER the form
    modified.append(fname)
    print(f"OK: {fname} ({orig_len}->{len(c)} chars, form@{idx}, </main>_after={has_main_before})")
