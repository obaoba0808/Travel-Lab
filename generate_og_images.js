// P0-4: Generate cropped 1200x630 WebP og:image files using sharp
...
const ogUrlMap = {}; // html_file -> og_url

async function processAll() {
    for (const [htmlFile, ogId, srcImg] of OG_IMAGES) {
        const srcPath = path.join(IMAGES_DIR, srcImg);
        const outPath = path.join(IMAGES_DIR, `${ogId}.webp`);
    ...
    for (const [htmlFile, ogId] of OG_IMAGES) {
        const fpath = path.join(BASE, htmlFile);
