...
for fname, pdf_key in PAGE_PDF_MAP.items():
    ...
    # Step 1: Remove ALL existing lead-related code (from any previous attempt)
    # Remove lead-inline sections (may be nested)
    while '<section class="lead-inline">' in c:
        c = re.sub(r'<section class="lead-inline">.*?</section>\s*</section>', '', c, count=1, flags=re.DOTALL)
    c = re.sub(r'<section class="lead-inline">.*?</section>', '', c, flags=re.DOTALL)
    
    # Remove inline CSS for lead
    c = re.sub(r'\n<style>\.lead-inline\{.*?</style>', '', c, flags=re.DOTALL)
    c = re.sub(r'\n<style>\n\.lead-inline\{.*?</style>', '', c, flags=re.DOTALL)
    
    # Remove our submitLeadForm script block
    c = re.sub(r'\n<script>\nconst WORKER_URL = "https://golightly-email\.happybird\.workers\.dev".*?</script>', '', c, flags=re.DOTALL)
    
    # Step 2: Find the BEST insertion point
    # Priority order:
    # 1. Before <!-- KLOOK or <ins class="klk-aff-widget" or klook-dynamic-banner
    # 2. Before </main> 
    # 3. Before <footer or footer div
    ...
    insert_markers = [
        ...
        ('</main>', FORM_HTML + '\n'),
        ('<footer', FORM_HTML + '\n'),
        ('<div class="footer"', FORM_HTML + '\n'),
    ]
    
    inserted = False
    for marker, replacement in insert_markers:
        if marker in c:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        verify = f.read()
    idx = verify.find('lead-inline')
    # Find what's right before it
    context = verify[max(0,idx-200):idx]
    
    modified.append(fname)
    print(f"OK: {fname} (pos={idx}, before=...{context[-80:]})")
